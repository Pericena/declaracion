...
async function predice() {
    ...
    if (img_.src !== "") {
        ...
        const highProbabilityResult = result.find(item => item.probability >= 0.3);

        if (highProbabilityResult) {
            document.getElementById('console').innerText = 'Es un: ' + highProbabilityResult.className;
        } else {
            document.getElementById('console').innerText = 'No se pudo identificar con alta probabilidad.';
        }
